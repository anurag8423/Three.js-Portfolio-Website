  ...
  return (
    <motion.div variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
      <Tilt options={{
        ...
        scale: 1,
        speed: 450
      }} className="bg-tertiary w-full p-5 rounded-2xl sm:w-[360px]">
        <div className="relative w-full h-[230px]">
          ...
          <div className="absolute inset-0 m-3 flex justify-end card-image_hover gap-2">
            ...
            <div onClick={() => window.open(deployment_link, "__blank")} className="black-gradient w-10 h-10 rounded-full flex justify-center items-center cursor-pointer">
              <img src={hyperlink} alt="deployment_link" className="w-1/2 h-1/2 object-contain" />
            </div>
          </div>
        </div>
        <div className="mt-3">
    ...
    <>
      ...
      <div className="mt-20 flex flex-wrap gap-7">
        {projects.map((project, index) => (
          <ProjectCard key={`project-${index}`} project={project} {...project} />
        ))}
      </div>
    </>
  )
